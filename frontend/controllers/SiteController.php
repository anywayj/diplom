...
class SiteController extends Controller
{
    ...
    {   
        ...
        /* $news = $document->find(".center_coloumn, head");*/
...
*/
    ...
    {      
        $beststud1 = "
        SELECT *
      /*  Students.Student_id,Students.Student_FIO, Group_academy.Group_name, science_work.name, science_work.rank_stud, progress_in_science.date, science_work.view */
        FROM (Students INNER JOIN (science_work 
        INNER JOIN progress_in_science ON science_work.science_id = progress_in_science.science_id) 
        ON Students.Student_id = progress_in_science.Student_id) INNER JOIN (Group_academy INNER JOIN Fixation_by_group 
        ON Group_academy.Group_id = Fixation_by_group.Group_id) ON Students.Student_id = Fixation_by_group.Student_id
        WHERE (((progress_in_science.date) Between '2018-01-01' And '2018-03-01') AND ((science_work.competition_stud)=True))
        ";

        $beststud = ScienceWork::findBySql($beststud1)->all();
    ...
    public function actionBeststud()
    {      
        ...
        $beststudd1 = "
        SELECT Students.Student_FIO, Sum(science_work.rank_stud) AS s1stud
        FROM Students INNER JOIN (science_work 
        INNER JOIN progress_in_science ON science_work.science_id = progress_in_science.science_id) 
        ON Students.Student_id = progress_in_science.Student_id
        WHERE (((progress_in_science.date) Between '2018-01-01' And '2018-06-01'))
        GROUP BY Students.Student_FIO, science_work.competition_stud
        HAVING (((science_work.competition_stud)=True));
        ";

        $beststudd = Students::findBySql($beststudd1)->all();

        $beststudd2 = "
        SELECT *
        FROM science_work WHERE science_work.competition_stud=True;
        
        ";

        $beststudd22 = ScienceWork::findBySql($beststudd2)->all();

        return $this->render('beststud', [
            'beststudd' => $beststudd,
            'beststudd22' => $beststudd22,
            'modelf' => $modelf,
        ]);
    }
    
    public function actionBestgroup()
    {      
        ...
        $bestgroup1 = "
        SELECT * FROM Science_work
        WHERE science_work.competition_group=True; 
        ";  

        $bestgroup2 = "
        SELECT * FROM public_work
        WHERE public_work.competition_group=True; 
        ";

        $bestgroup = ScienceWork::findBySql($bestgroup1)->all();
        $bestgroupp = PublicWork::findBySql($bestgroup2)->all();
        

       

        return $this->render('bestgroup', [
            'bestgroup' => $bestgroup,
            'bestgroupp' => $bestgroupp,
            'modelf' => $modelf,
        ]);
    }
